...
<div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[radial-gradient(circle,rgba(124,255,0,0.1)_0%,transparent_70%)] -z-10 pointer-events-none"></div>
    ...
    <div class="metal-card p-6 md:p-14 rounded-2xl relative overflow-hidden shadow-2xl">
        ...
        <div class="relative z-10">
            ...
            <form action="{{ route('login.post') }}" method="POST" @submit="loading = true" class="space-y-8">
                ...
                @if($errors->any())
                    <div class="bg-brand-neon/10 border border-brand-neon/20 text-brand-neon text-[10px] p-4 rounded-2xl font-bold uppercase tracking-widest text-center">
                        {{ $errors->first() }}
                    </div>
                @endif
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        <div class="absolute left-6 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-brand-neon transition-colors pointer-events-none">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                        </div>
                        <input type="email" name="email" required placeholder="admin@ventuz.com" 
                            class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 pl-16 pr-6 text-sm font-medium focus:outline-none focus:border-brand-neon/50 transition-all">
                    </div>
                </div>
                ...
                <div class="space-y-3" x-data="{ show: false }">
                    <div class="flex items-center justify-between px-1">
                        ...
                        <a href="#" class="text-[10px] text-brand-neon font-black uppercase tracking-widest hover:brightness-125">Lupa Password?</a>
                    </div>
                    <div class="relative group">
                        <div class="absolute left-6 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-brand-neon transition-colors pointer-events-none">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <input :type="show ? 'text' : 'password'" name="password" required placeholder="••••••••" 
                            class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 pl-16 pr-14 text-sm font-medium focus:outline-none focus:border-brand-neon/50 transition-all">
                ...
                <div class="flex items-center gap-3 px-1">
                    <input type="checkbox" name="remember" class="w-4 h-4 accent-brand-neon rounded bg-white/5 border-white/10">
            ...
            <div class="mt-12 pt-8 border-t border-white/5 text-center">
                <p class="text-gray-500 text-xs font-medium">Belum punya akun? 
                    <a href="{{ route('register') }}" class="text-white font-bold hover:text-brand-neon transition-colors">Daftar Akun</a>
                </p>
            </div>
        </div>
    </div>
