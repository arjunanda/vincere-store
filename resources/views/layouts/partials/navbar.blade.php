<!-- Navbar -->
<nav class="sticky top-0 z-50 border-b border-white/[0.06] backdrop-blur-xl" style="background: rgba(18, 20, 26, 0.96);"
    x-data="{ mobileMenu: false, showSearch: false }" aria-label="Navigasi Utama">
    ...
    <div class="max-w-7xl mx-auto px-6 h-[68px] flex items-center justify-between relative">
        ...
        <div class="flex items-center gap-3 flex-shrink-0">
            ...
            <div class="hidden lg:flex flex-col items-center space-y-0">
                <span
                    class="text-lg md:text-xl font-black tracking-tighter uppercase text-white leading-none">Vincere</span>
                <span
                    class="text-[8px] md:text-[9px] font-black tracking-[0.4em] uppercase text-brand-neon leading-none ml-[0.4em]">Store</span>
            </div>
        </div>
        ...
        <div class="lg:hidden absolute left-[45%] -translate-x-1/2 flex flex-col items-center space-y-0">
            ...
            <span
                class="text-[9px] font-black tracking-[0.4em] uppercase text-brand-neon leading-none ml-[0.4em]">Store</span>
        </div>

        <!-- ── Col 2: Desktop Nav Links (Center) ── -->
        <nav class="hidden lg:flex items-center gap-1 flex-1 justify-center" aria-label="Menu Utama">
            <a href="/" title="Kembali ke Beranda" class="relative px-4 py-2 rounded-xl text-[14px] font-semibold tracking-wide transition-all duration-200
                    {{ Route::is('index')
    ...
    : 'text-white/60 hover:text-white hover:bg-white/[0.05]' }}">
            ...
            <a href="{{ route('games.index') }}" title="Lihat Daftar Game Terlengkap" class="relative px-4 py-2 rounded-xl text-[14px] font-semibold tracking-wide transition-all duration-200 whitespace-nowrap
    ...
    : 'text-white/60 hover:text-white hover:bg-white/[0.05]' }}">
            ...
            <a href="{{ route('check.transaction') }}" title="Cek Status Pesanan Kamu" class="relative px-4 py-2 rounded-xl text-[14px] font-semibold tracking-wide transition-all duration-200 whitespace-nowrap
    ...
    : 'text-white/60 hover:text-white hover:bg-white/[0.05]' }}">
            ...
            <a href="{{ route('news.index') }}" title="Berita Gaming dan Promo Terbaru" class="relative px-4 py-2 rounded-xl text-[14px] font-semibold tracking-wide transition-all duration-200 whitespace-nowrap
                    {{ Route::is('news.index')
    ? 'text-brand-neon bg-brand-neon/10'
    : 'text-white/60 hover:text-white hover:bg-white/[0.05]' }}">
                Promo & Berita
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <!-- Search Button -->
            <button @click="showSearch = !showSearch" aria-label="Cari Game" class="w-10 h-10 flex items-center justify-center rounded-xl border transition-all duration-200
                    bg-white/[0.04] border-white/10 text-white/60
                    hover:bg-brand-neon/10 hover:border-brand-neon/40 hover:text-brand-neon">
                <svg x-show="!showSearch" x-cloak class="w-[18px] h-[18px]" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
            ...
            @guest
                <!-- Login -->
                <a href="{{ route('login') }}" title="Masuk ke Akun Kamu" class="hidden lg:inline-flex items-center gap-2 h-10 px-5 rounded-xl text-[13.5px] font-semibold
            ...
            @else
                <!-- Dashboard -->
                <a href="{{ route('dashboard.index') }}" title="Buka Dashboard Member" class="hidden lg:inline-flex items-center gap-2 h-10 px-5 rounded-xl text-[13.5px] font-bold
            ...
            <!-- Mobile Toggle -->
            <button @click="mobileMenu = !mobileMenu" aria-label="Menu Mobile"
    ...
    <!-- Search Bar Dropdown -->
    <div x-show="showSearch" x-cloak x-transition:enter="transition ease-out duration-250"
        x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
        x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 -translate-y-2" class="border-t border-white/[0.06] py-4 px-6"
        ...
        <form action="{{ route('games.index') }}" method="GET" class="max-w-2xl mx-auto relative">
            ...
            <input type="text" name="search" placeholder="Cari game favoritmu..." autofocus class="w-full bg-white/[0.04] border border-white/10 rounded-xl py-3 pl-12 pr-20
            ...
            <div class="absolute right-4 top-1/2 -translate-y-1/2">
                <kbd
                    class="text-[10px] font-bold text-white/30 uppercase tracking-widest bg-white/[0.06] border border-white/10 px-2 py-1 rounded-md">Enter</kbd>
            </div>
        </form>
    </div>

    <!-- Mobile Menu -->
    <div x-show="mobileMenu" x-cloak x-transition:enter="transition ease-out duration-250"
        x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
        x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 -translate-y-2"
        class="lg:hidden border-t border-white/[0.06] px-6 py-5 space-y-2" style="background: rgba(12, 14, 18, 0.98);">
        ...
        <div class="pt-4 border-t border-white/[0.06] space-y-2">
            ...
            @else
                <a href="{{ route('dashboard.index') }}" class="flex items-center justify-center gap-2 w-full py-3 rounded-xl font-bold text-[14px]
