...
<!-- CS Floating Menu -->
<div class="whatsapp-float hidden md:flex" x-data="{ open: false }">
    <!-- Menu Items -->
    <div class="flex flex-col gap-4 mb-2" 
        ...
        <!-- Instagram -->
        <a href="{{ $webSettings['contact_ig'] ?? '#' }}" target="_blank" class="cs-menu-item cs-ig group">
...
@php
    $waNumber = preg_replace('/[^0-9]/', '', $webSettings['contact_wa'] ?? '');
    if (str_starts_with($waNumber, '0')) $waNumber = '62' . substr($waNumber, 1);
@endphp
        <!-- WhatsApp -->
        <a href="https://wa.me/{{ $waNumber }}?text=Halo%20Admin%20{{ urlencode($webSettings['web_title'] ?? 'Ventuz Store') }},%20saya%20ingin%20bertanya%20tentang..." target="_blank" class="cs-menu-item cs-wa group">
            <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24">
