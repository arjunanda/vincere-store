...
@section('content')
        ...
        <form action="{{ route('check.transaction') }}" method="GET">
            <div class="flex gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="order_id" value="{{ request('order_id') }}" placeholder="Masukkan Order ID..."
                        required
    ...
    @if(request()->has('order_id'))
        <div x-data="{ show: false }" x-init="setTimeout(() => show = true, 80)" x-show="show"
            ...
            @if($transaction)
                ...
                <div class="rounded-2xl border overflow-hidden
                                ...
                                @else border-white/[0.07] @endif" style="background:#1a1d27">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                            <div
                                class="w-14 h-14 rounded-xl overflow-hidden border border-white/[0.07] flex-shrink-0 bg-white/[0.04]">
                                <img src="{{ asset('storage/' . $transaction->game->image) }}" class="w-full h-full object-cover"
                                    alt="{{ $transaction->game_name }}">
                            </div>
                            <div class="min-w-0">
                                <p class="text-[13px] font-black text-white uppercase truncate">{{ $transaction->game_name }}</p>
                                <p class="text-[11px] font-bold text-brand-neon uppercase tracking-wide">
                                    {{ $transaction->variant_name }}</p>
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @foreach($transaction->input_data as $label => $value)
                                <div
                                    class="flex items-center justify-between px-4 py-3 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                            ...
                            <div
                                ...
                                <span
                                    class="text-[13px] font-black text-white">{{ $transaction->created_at->format('d M Y, H:i') }}</span>
                            </div>
                        </div>
            ...
            @else
                ...
                <div class="rounded-2xl border border-red-500/20 overflow-hidden" style="background:#1a1d27">
                    <div class="p-8 text-center space-y-4">
                        <div
                            class="w-14 h-14 mx-auto rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-400">
