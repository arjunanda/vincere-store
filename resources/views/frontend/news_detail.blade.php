...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
    ...
    <div class="lg:col-span-2 space-y-12">
        ...
        <div class="space-y-6">
            <div class="flex items-center gap-4">
                <span class="px-4 py-1.5 rounded-xl bg-brand-red text-white text-xs font-black uppercase tracking-[0.2em] shadow-[0_0_15px_rgba(225,29,72,0.4)]">
                    {{ $article->type }}
                </span>
            ...
            <h1 class="text-4xl md:text-6xl font-black italic uppercase tracking-tighter leading-none text-white">
                {{ $article->title }}
            </h1>
        </div>
        ...
        <div class="pt-12 border-t border-white/5 flex items-center justify-between">
            ...
            <div class="flex gap-4">
                <button class="w-10 h-10 rounded-xl glass-dark flex items-center justify-center hover:bg-brand-red transition-all">
    ...
    <div class="space-y-12">
        <div class="space-y-8">
            <h5 class="text-xs font-black uppercase tracking-[0.4em] text-brand-red">Berita Lainnya</h5>
            <div class="space-y-6">
                ...
                <a href="{{ route('news.detail', $related->slug) }}" class="group flex gap-6 items-start">
                    ...
                    <div class="space-y-2">
                        <h6 class="text-sm font-bold uppercase tracking-tight italic group-hover:text-brand-red transition-colors line-clamp-2">{{ $related->title }}</h6>
        ...
        <a href="{{ route('games.index') }}" class="block metal-card p-8 rounded-xl relative overflow-hidden group">
            <div class="relative z-10 space-y-6">
                <span class="text-[10px] font-black uppercase tracking-widest text-brand-red">Special Promo</span>
                <h4 class="text-2xl font-black italic uppercase leading-tight tracking-tighter">Top-up Game Favoritmu Sekarang!</h4>
                <button class="btn-metal w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-widest">Gaskeun!</button>
            </div>
            <div class="absolute -right-4 -bottom-4 w-32 h-32 bg-brand-red opacity-10 blur-3xl group-hover:opacity-20 transition-opacity"></div>
        </a>
    </div>
</div>
@endsection
