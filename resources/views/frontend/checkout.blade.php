...
@section('content')
    <div class="space-y-6 md:space-y-10">
        ...
        <div class="metal-card rounded-[2.5rem] overflow-hidden border-white/5 shadow-2xl relative">
            ...
            <div class="p-6 md:p-12 space-y-8 md:space-y-12">
                ...
                <div class="text-center space-y-3">
                    <h1
                        class="text-3xl md:text-5xl font-black italic uppercase tracking-tighter metallic-text leading-none">
                        Invoice <span class="text-brand-red">Pembelian</span>
                    </h1>
                    <div class="flex flex-col items-center gap-1">
                        <div class="flex items-center justify-center gap-2 group cursor-pointer active:scale-95 transition-transform"
                            ...
                            <svg class="w-3.5 h-3.5 text-gray-600 group-hover:text-brand-red transition-colors" fill="none"
                        ...
                        @if($transaction->payment_status === 'pending')
                            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-brand-red animate-pulse">Menunggu Pembayaran</p>
                    ...
                    <div class="grid grid-cols-1 gap-5 md:gap-6">
                        <div class="flex justify-between items-center px-1">
                            <span class="text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Game /
                                Product</span>
                            <span
                                class="text-xs md:text-base font-black text-white italic uppercase">{{ $game->name }}</span>
                        </div>
                        <div class="flex justify-between items-center px-1">
                            <span class="text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-widest">Item /
                                Variant</span>
                            <span
                                class="text-xs md:text-base font-black text-brand-red italic uppercase">{{ $variant->name }}</span>
                        </div>
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-3 group cursor-pointer active:scale-95 transition-all"
                                ...
                                <span class="text-2xl md:text-4xl font-black italic text-white leading-none">
                                    <span
                                        class="text-brand-red text-lg md:text-xl mr-1">Rp</span>{{ number_format($transaction->total_price, 0, ',', '.') }}
                                </span>
                                <div
                                    class="p-2 bg-white/5 rounded-lg text-gray-500 group-hover:text-brand-red transition-colors">
                ...
                @if($transaction->payment_status === 'pending' || $transaction->payment_status === 'verif')
                    <div class="space-y-6">
                        <p class="text-center text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">Instruksi
                            Pembayaran</p>
                        <div
                            class="border-2 border-dashed border-white/10 rounded-[2rem] p-6 md:p-10 bg-white/[0.01] relative overflow-hidden group hover:border-brand-red/30 transition-all shadow-inner">
                            <div class="flex flex-col items-center gap-6">
                                <div class="text-center space-y-4">
                                    <p
                                        class="text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-widest leading-relaxed px-4">
                                        Anda memilih bayar via <span class="text-brand-red">{{ $paymentMethod->name }}</span>,
                                    ...
                                    <div class="text-center space-y-4 w-full">
                                        <div class="space-y-4">
                                            <div class="flex items-center justify-center gap-3 group cursor-pointer active:scale-95 transition-all"
                                                ...
                                                <h3 class="text-2xl md:text-4xl font-black italic text-white tracking-tight break-all leading-none"
                                                    id="acc_number">
                                                    {{ $paymentMethod->account_number }}
                                                </h3>
                                                <div
                                                    class="p-2 bg-white/5 rounded-xl text-gray-500 group-hover:text-brand-red transition-colors shrink-0">
                        ...
                        @if($transaction->payment_status === 'pending')
                            <div class="bg-brand-red/5 border border-white/5 p-4 rounded-2xl flex items-center gap-4">
                                <div
                                    class="w-8 h-8 bg-brand-red/20 rounded-full flex items-center justify-center text-brand-red shrink-0">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <p class="text-[10px] md:text-xs text-gray-400 font-medium italic">
            ...
            <div class="metal-card rounded-3xl overflow-hidden border-white/5 shadow-2xl">
                <div class="p-8 md:p-10 space-y-8">
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 bg-brand-red/10 rounded-2xl flex items-center justify-center text-brand-red border border-brand-red/20 shadow-lg shadow-brand-red/10">
                        ...
                        <div class="space-y-1">
                            <h3 class="text-xl font-black italic uppercase text-white leading-tight">Upload Bukti</h3>
                    ...
                    <form action="{{ route('checkout.proof', $transaction->order_id) }}" method="POST"
                        ...
                        <div class="relative group">
                            ...
                            <label for="proof_of_payment"
                                class="flex flex-col items-center justify-center w-full aspect-video rounded-[2rem] border-2 border-dashed border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-brand-red/30 transition-all cursor-pointer group relative overflow-hidden">
                                ...
                                <div x-show="!imagePreview" class="flex flex-col items-center text-center px-10 space-y-4">
                                    <div
                                        class="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center text-gray-600 group-hover:text-brand-red transition-all">
                        ...
                        <button type="submit"
                            class="btn-metal relative w-full py-5 rounded-2xl font-black uppercase tracking-[0.3em] text-[11px] group disabled:opacity-50 shadow-2xl shadow-brand-red/20 overflow-hidden"
        ...
        @elseif($transaction->payment_status === 'verif' && $transaction->proof_of_payment)
            <div class="bg-brand-red/10 border-2 border-brand-red/30 p-8 rounded-3xl text-center space-y-4 shadow-2xl">
                <div class="flex justify-center">
                    <div
                        class="w-16 h-16 bg-brand-red text-white rounded-full flex items-center justify-center animate-bounce shadow-xl shadow-brand-red/40">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-1">
                    <h4 class="text-xl font-black italic uppercase text-white">Bukti Terkirim!</h4>
                    <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">Tunggu sekitar 3-5 menit untuk
                        verifikasi otomatis.</p>
                </div>
                <a href="{{ route('index') }}"
                    class="inline-block text-[10px] font-black text-brand-red uppercase tracking-[0.3em] mt-4 hover:text-white transition-colors">Kembali
                    ke Beranda</a>
            </div>
        @endif
        ...
        <div class="flex flex-col items-center gap-4 pt-4">
            <p class="text-[10px] text-gray-600 font-bold uppercase tracking-widest italic">* Simpan halaman ini atau catat
                ID Transaksi Anda</p>
