    ...
    <div class="space-y-8">
            ...
            <div class="p-6 md:p-10 space-y-6">
                ...
                <div class="text-center space-y-2">
                    ...
                    <div class="flex items-center justify-center gap-3 group cursor-pointer"
                        ...
                        <div class="p-1.5 rounded-lg bg-white/10 border border-white/10 group-hover:bg-brand-neon/15 group-hover:border-brand-neon/40 transition-all">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-3 py-2">
                    ...
                    <div class="flex items-center gap-3 group cursor-pointer"
                        ...
                        <div class="p-1.5 rounded-lg bg-white/10 border border-white/10 group-hover:bg-brand-neon/15 group-hover:border-brand-neon/40 transition-all shrink-0">
                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-gray-400 group-hover:text-brand-neon transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                        </div>
                    ...
                    <div class="text-center space-y-4">
                        ...
                        @if($paymentMethod->type === 'qris')
                            <p class="text-xs text-gray-400">
                                Bayar via <span class="text-brand-neon font-bold">{{ $paymentMethod?->code ?? $paymentMethod?->name ?? '—' }}</span>
                            </p>
                            <div class="bg-white p-4 rounded-2xl inline-block">
                                <img src="{{ asset('storage/' . $paymentMethod->qris_image) }}" class="w-48 md:w-56 h-auto" alt="QRIS">
                            </div>
                            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest animate-pulse">Scan QR di atas untuk bayar</p>
                        @else
                            <p class="text-xs text-gray-400">
                                Bayar via <span class="text-brand-neon font-bold">{{ $paymentMethod?->code ?? $paymentMethod?->name ?? '—' }}</span> &mdash; transfer tepat sesuai tagihan
                            </p>
                            <div class="relative bg-gradient-to-b from-white/[0.05] to-transparent border border-white/5 rounded-xl p-4 space-y-4 max-w-xs mx-auto shadow-lg shadow-brand-neon/5">
                                <div class="absolute top-0 left-0 w-full h-full rounded-xl bg-gradient-to-b from-brand-neon/[0.03] to-transparent pointer-events-none"></div>
                                <p class="text-xl md:text-2xl font-black text-white tracking-tight relative">{{ $paymentMethod->bank_name ?: $paymentMethod->name }}</p>
                                <div class="flex flex-wrap justify-center items-center gap-2 relative">
                                    <span class="text-white font-bold text-lg md:text-xl break-all tracking-wider">{{ $paymentMethod->account_number }}</span>
                                    <div class="group cursor-pointer shrink-0 p-1.5 rounded-lg bg-white/10 border border-white/10 hover:bg-brand-neon/15 hover:border-brand-neon/40 transition-all hover:shadow-lg hover:shadow-brand-neon/20"
                                        ...
                                        <svg class="w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-brand-neon transition-colors" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                        </svg>
                                ...
                                <p class="text-xs text-gray-400 font-bold uppercase tracking-wider relative">A/N <span class="text-white">{{ $paymentMethod->account_name }}</span></p>
                            </div>
                        @endif
                    </div>
                    ...
                    @if($transaction->payment_status === 'pending')
                        <div class="space-y-3 pt-4">
                            <div class="bg-brand-neon/5 border border-white/5 p-4 rounded-xl text-center">
                                <p class="text-xs text-gray-400">Kadaluarsa dalam <span class="text-white font-bold">24 Jam</span> jika tidak dibayar</p>
                            </div>
            ...
            <div class="metal-card rounded-3xl overflow-hidden border-white/5 shadow-xl">
                <div class="p-6 md:p-8 space-y-6">
                    ...
                    <form action="{{ route('checkout.proof', $transaction->order_id) }}" method="POST"
                        ...
                        @csrf
                        @if($errors->any())
                            <div class="bg-red-500/10 border border-red-500/20 text-red-500 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest">
                                {{ $errors->first() }}
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <label for="proof_of_payment"
                                ...
                                <div x-show="!imagePreview" class="flex flex-col items-center py-8 space-y-2">
                                    <div class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center text-gray-500">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                        </svg>
                                    </div>
                                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Klik untuk upload bukti</p>
                                </div>
                            </label>
                        </div>
        ...
        @elseif($transaction->payment_status === 'verif' && $transaction->proof_of_payment)
            <div class="bg-brand-neon/10 border border-brand-neon/30 p-6 rounded-2xl text-center space-y-3">
                ...
                <a href="{{ route('index') }}" class="inline-block text-[10px] font-bold text-brand-neon uppercase tracking-widest mt-2">Kembali ke Beranda</a>
            </div>
        @endif

        <!-- Footer -->
        <div class="text-center space-y-3 pt-2">
            <p class="text-[10px] text-gray-600 font-bold uppercase tracking-widest">* Simpan halaman ini atau catat ID Transaksi</p>
