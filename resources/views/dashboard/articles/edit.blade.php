...
<div class="max-w-5xl mx-auto space-y-8">
    <div>
        ...
        <h1 class="text-3xl font-black  uppercase tracking-tight text-white">Edit <span class="text-brand-neon">Artikel</span></h1>
    </div>

    <form action="{{ route('dashboard.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-8" novalidate>
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="stat-card space-y-6">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title" value="{{ old('title', $article->title) }}" required placeholder="Contoh: Update Patch Note MLBB v1.2" class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-brand-neon/50 text-white font-bold text-lg">
                    </div>

                    <div class="space-y-3">
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Konten Lengkap</label>
                        <textarea name="content" id="content-editor" rows="15" class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-6 px-6 focus:outline-none focus:border-brand-neon/50 text-white font-medium leading-relaxed" placeholder="Tulis artikel di sini...">{{ old('content', $article->content) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="stat-card space-y-6 sticky top-24">
                    <div class="space-y-3">
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Status Publikasi</label>
                        <select name="is_active" required class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-brand-neon/50 text-white font-medium appearance-none">
                            <option value="1" {{ $article->is_active ? 'selected' : '' }} class="bg-black">Published</option>
                            <option value="0" {{ !$article->is_active ? 'selected' : '' }} class="bg-black">Draft</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <select name="type" required class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-brand-neon/50 text-white font-medium appearance-none">
                            <option value="berita" {{ $article->type == 'berita' ? 'selected' : '' }} class="bg-black">Berita / Update</option>
                            <option value="promo" {{ $article->type == 'promo' ? 'selected' : '' }} class="bg-black">Promo / Event</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-3" x-data="{ 
                        ...
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Thumbnail Artikel (Opsional)</label>
                        ...
                        <p class="text-[9px] text-gray-500 mt-1  text-center">* Biarkan kosong jika tidak ingin mengubah gambar.</p>
                    </div>

                    <div class="space-y-3">
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Ringkasan (Excerpt)</label>
                        <textarea name="excerpt" rows="3" class="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-4 px-6 focus:outline-none focus:border-brand-neon/50 text-white text-xs leading-relaxed" placeholder="Ringkasan singkat artikel...">{{ old('excerpt', $article->excerpt) }}</textarea>
                    </div>
                    ...
                    <button type="submit" class="btn-metal w-full py-5 rounded-2xl font-black uppercase tracking-widest text-sm shadow-lg shadow-brand-neon/20">
                        <span class="btn-text">Perbarui Artikel</span>
                    </button>
                </div>
            </div>
        </div>
