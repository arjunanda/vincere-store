...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <a href="{{ route('dashboard.payments') }}" class="text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-brand-red flex items-center gap-2 mb-2 transition-colors">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                Kembali
            </a>
            <h1 class="text-2xl font-black italic uppercase tracking-tight">Tambah <span class="text-brand-red">Metode</span></h1>
        </div>
    </div>
    ...
    <form action="{{ route('dashboard.payments.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" 
        ...
        <div class="stat-card">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Nama Provider (BCA, OVO, dll) <span class="text-brand-red">*</span></label>
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Kode Unik (Slug/ID) <span class="text-brand-red">*</span></label>
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Biaya Layanan (Fee) <span class="text-brand-red">*</span></label>
                ...
                <div class="space-y-2">
                    ...
                    @error('account_number') <p class="text-brand-red text-[10px] italic mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    @error('account_name') <p class="text-brand-red text-[10px] italic mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="stat-card space-y-4">
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-500 block">Logo Metode <span class="text-brand-red">*</span></label>
            ...
            <div class="stat-card space-y-4 border-brand-red/10" x-show="paymentType === 'qris'" x-cloak x-transition>
                <label class="text-[10px] font-black uppercase tracking-widest text-brand-red block">Gambar QRIS (Wajib jika QRIS)</label>
                <div class="flex flex-col items-center gap-4 p-6 bg-white/[0.02] rounded-2xl border border-dashed border-white/10 group hover:border-brand-red/50 transition-all">
                    <div class="w-20 h-20 rounded-xl bg-white/5 flex items-center justify-center overflow-hidden border border-white/10">
                        ...
                        <template x-if="!qrisPreview">
                            <svg class="w-8 h-8 text-brand-red/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h2M5 8V2.1m12.9 15.9l1.4 1.4M5 4.1L4.1 5m15.9 2.1l1.4 1.4M4.9 19.9l1.4 1.4" /></svg>
                        </template>
                    </div>
                    <label class="cursor-pointer">
